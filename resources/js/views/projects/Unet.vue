      ...
      <div class="container">
        <div class="row pb-5">
          <div class="col-xl-9 pb-4">
            <p>Both these design choices, combined with the previous aspects I highlighted about the architecture, mean that a UNet can vastly outperform a traditional CNN when used for segmentation tasks. Additionally, when using a traditional approach, we need to create patches from our input that are then classified one at a time. When using a UNet we can classify slice by slice instead of patch by patch, which makes the UNet much, much faster. While for this specific segmentation task, we aren’t necessarily facing a time constraint, the speedup in other contexts (ie. street segmentation in self-driving cars) can be game changing. I have uploaded all the code for this project to my <a href="https://github.com/toehmler/nunet">GitHub</a> if you’d like to take a look, or see some of the results I’ve been able to achieve. I am still actively working on the project so the repo is updated frequently.</p>
          </div>
        </div>
      </div>
    </div>
